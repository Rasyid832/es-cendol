...
<body class="bg-[#0B132B] text-white min-h-screen font-sans flex flex-col">
    ...
    <script>
        ...
        const supervisorPeer = new Peer(supervisorPeerID, { debug: 1 });
        let dataConnections = [];

        function getOpenConnections() {
            dataConnections = dataConnections.filter(conn => conn && !conn.destroyed && conn.open);
            return dataConnections;
        }

        function sendToStudent(payload, successMessage) {
            const connections = getOpenConnections();
            if (!connections.length) {
                alert('Gagal mengirim! Koneksi siswa belum siap.');
                return false;
            }
        ...
        supervisorPeer.on('open', (id) => {
            console.log('Supervisor Peer siap:', id);
        });

        supervisorPeer.on('connection', (conn) => {
            console.log('Koneksi data siswa masuk:', conn.peer);
            if (!dataConnections.some(item => item.peer === conn.peer)) {
                dataConnections.push(conn);
            }

            conn.on('open', () => {
                console.log('Koneksi data siswa siap:', conn.peer);
                if (!dataConnections.some(item => item.peer === conn.peer)) {
                    dataConnections.push(conn);
                }
            });
            ...
            conn.on('data', (data) => {
                if (data && data.type === 'STUDENT_READY') {
                    console.log('Siswa siap menerima kontrol.');
                }
            });

            conn.on('close', () => {
                dataConnections = dataConnections.filter(item => item !== conn);
            });
        ...
        supervisorPeer.on('error', (error) => {
            console.error('Supervisor Peer error:', error);
        });

        function sendWarning(text) {
            sendToStudent({ type: 'WARNING', message: text }, 'Peringatan berhasil dikirim ke layar siswa.');
        }
        ...
        function sendCustomLog(e) {
            ...
            if (sendToStudent({ type: 'WARNING', message: msg }, 'Pesan berhasil dikirim ke layar siswa.')) {
                input.value = '';
            }
        }
        ...
        async function stopExam() {
            ...
            try {
                ...
                sendToStudent({ type: 'STOP_EXAM' }, 'Perintah penghentian dikirim ke siswa.');
