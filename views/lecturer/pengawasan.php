...
try {
    ...
    $stmt_logs_count = $pdo->prepare("
        ...
        WHERE s.room_id = :room_id
    ");
    $stmt_logs_count->execute(['room_id' => $actual_room_id]);
    $total_logs = $stmt_logs_count->fetchColumn() ?: 0;
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div class="neon-border-wrapper">
                <div class="cyber-card bg-white border border-slate-200 p-5 shadow-lg flex items-center gap-4">
                    ...
                    <div>
                        <p class="text-xs text-slate-500 text-sub-title">Total Record Telemetri</p>
                        <p class="text-2xl font-bold text-slate-900 text-main-title"><?= $total_logs ?> <span class="text-xs font-normal text-slate-400">Log</span></p>
                    </div>
                </div>
            </div>
                                ...
                                <?php if (!empty($joined_students)): ?>
                                    <?php foreach ($joined_students as $student): ?>
                                        <tr class="hover:bg-slate-50 transition">
                                            ...
                                            <td class="p-3 font-mono-code text-indigo-600 font-semibold"><?= htmlspecialchars($student['log_count'] ?? 0) ?> Record</td>
                                            <td class="p-3 text-right">
                                                <div class="inline-flex items-center gap-2">
                                                    ...
                                                    <a href="detailpengawasan.php?student_id=<?= urlencode($student['id'] ?? 0) ?>" class="p-1.5 bg-indigo-600/10 hover:bg-indigo-600/20 text-indigo-600 rounded-lg transition" title="Detail Telemetri">
                ...
                <div class="cyber-card bg-white border border-slate-200 p-6 shadow-xl space-y-4">
                    <div class="flex items-center justify-between border-b border-slate-200 pb-3" id="stream-header-border">
                        <h3 class="text-xs font-bold text-indigo-600 uppercase tracking-wider flex items-center gap-2" id="section-2-title">
                            <i class="fa-solid fa-stream"></i> Telemetry Stream
                        </h3>
                            ...
                            <?php foreach ($recent_logs as $log): ?>
                                <div class="bg-slate-50 border border-slate-200 p-3 rounded-xl text-xs space-y-1 stream-log-item">
                                    ...
                                    <div class="text-[10px] text-indigo-600 bg-indigo-50 px-2 py-1 rounded border border-indigo-100 mt-1 font-mono-code">
                                        <i class="fa-solid fa-code text-[9px]"></i> Log ke-<?= htmlspecialchars($log['id'] ?? '-') ?> diterima
                                    </div>
                                </div>
                            <?php endforeach; ?>
                        <?php else: ?>
                            <p class="text-center text-slate-400 text-xs py-10">Belum ada aktivitas telemetri yang tercatat.</p>
                        <?php endif; ?>
                    </div>
                </div>
            </div>
