...
try {
    // Menyaring HANYA room yang diarsipkan (status = 'archived')
    $sql = "SELECT 
                r.id,
                r.subject_name,
                r.room_code,
    ...
    $error_message = "Gagal mengambil data arsip: " . $e->getMessage();
}
?>
<!DOCTYPE html>
<html lang="id">
<head>
    ...
    <title>Arsip Ujian - CodeProcess</title>
        ...
        <div class="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 border-b border-slate-200 pb-5 transition-colors" id="header-border">
            <div>
                <div class="flex items-center gap-3 mb-1">
                    <h2 class="text-2xl font-bold text-slate-900 tracking-tight text-main-title transition-colors">Arsip Ujian</h2>
                ...
                <p class="text-xs text-slate-500 text-sub-title transition-colors">Daftar room ujian lama yang telah diarsipkan untuk menjaga kerapian histori.</p>
            </div>
                                    ...
                                    <tr class="cyber-tr hover:bg-slate-50/80 transition-colors">
                                        ...
                                        <td class="px-6 py-4 text-center">
                                            <div class="flex items-center justify-center gap-2">
                                                ...
                                                <a href="../../controllers/controller_room_action.php?action=restore&id=<?= $room['id'] ?>&from=archive" onclick="return confirm('Pulihkan room ini ke status inactive?')" class="text-xs font-semibold bg-emerald-600 hover:bg-emerald-500 text-white px-3 py-2 rounded-xl transition-all shadow-md inline-flex items-center gap-1.5">
                                                    <i class="fa-solid fa-rotate-left"></i> Pulihkan
                                                </a>
                                                <a href="../../controllers/controller_room_action.php?action=delete&id=<?= $room['id'] ?>&from=archive" onclick="return confirm('Hapus permanen room ini beserta seluruh log siswa?')" class="text-xs font-semibold bg-rose-600 hover:bg-rose-500 text-white px-3 py-2 rounded-xl transition-all shadow-md inline-flex items-center gap-1.5">
